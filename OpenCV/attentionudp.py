...
class Face:
    ...
    def printFace(self): #reactretail code
        
        print("Age="+str(self.age));
        

    def updateFace(self, width, height, xpt, ypt,satisfied):
...
def detect_and_draw(img ,cascade):
    print "okokaoktaokatokaotkaotkt"
        ...
        for ((x, y, w, h), n) in faces:
            ...
            if (len(trackedFaces) > 0):
                ...
                for f in trackedFaces:
                    if (mf.age <100):
                        hexdata = "203555";
                    elif ((mf.age > 100) & (mf.age < 200)):
                        hexdata = "209555";
                    elif ((mf.age > 200) & (mf.age < 500)):
                        hexdata = "2014555";
                    elif (mf.age > 500):
                        hexdata = "2021055";
                    #target
                    ...
                    PORTNUM = 50000
                    #UDP packet hex content
                    PACKETDATA = 'hexdata'.decode('hex')
                    #initialize and connect socket
                    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM,0)
                    s.connect((IPPADDR, PORTNUM))
                    s.send(PACKETDATA)
                    s.close()
...
def AgreementPoints(mf):
    if (mf.isNodding()): att++;
    elif (mf.isShaking()): att--;
    else pass;
    Print ("agreement" + str(att));
